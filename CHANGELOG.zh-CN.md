...
- 新增自定义专家管理：支持创建、编辑、复制内置专家、选择头像与 Emoji，以及来源和启用状态筛选；自定义专家可从 @ 候选、聊天入口和召唤工具调用。
- 专家删除改为永久删除并立即释放名额，自动清理旧版软删除记录；名称冲突保留设置页改名和删除入口，不再导致整张名册不可用。
- 加强配置修订号和名册一致性校验，拒绝无效启用项，防止在途旧查询及延迟写入回执覆盖最新结果。
- 编辑冲突时保留草稿，支持读取并核对最新内容后再保存；其他窗口删除或改名释放名称后，旧本地名册不再阻止合法保存。
- 嵌套目录专家的查看、复制与召唤均读取名册选中的实际文件，避免正文缺失或读到同名专家的错误 persona。
- 优化聊天入口间距、下拉样式、蓝色焦点反馈和 Escape 分层处理；提示词预览使用原生弹窗，自定义引用标签与内置保持统一图标和纯名称。
- 修复自动更新请求的并发锁空档，并让发布校验兼容 Windows CRLF 换行。
