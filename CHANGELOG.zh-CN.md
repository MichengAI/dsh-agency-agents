...
- 支持 DeepSeek Harness 0.1.7-rc.2，并继续兼容此前已支持的 RC 版本。

## 1.0.4 - 2026-09-25

### 问题修复

- 在 Linux 全局安装或 Docker 中的 DSH 0.1.7-rc.1 上也能正常启动，并继续读取当前设置。

## 1.0.3 - 2026-09-24

### 体验优化

- 支持 DeepSeek Harness 0.1.7-rc.1，并继续兼容此前已支持的 RC 版本。
- 不再支持 Alpha 版本。

### 问题修复

- 在 DSH 0.1.7-rc.1 上仍能正确读取当前设置。

## 1.0.2 - 2026-09-23

### 体验优化

- 支持 DSH 0.1.7，并继续兼容此前已支持的版本。
- 聊天里选择专家和专家团时会显示头像。新建自定义专家不必再填召唤图标。
- 聊天菜单和设置页的专家团列表会保持同步。
- 专家和专家团的数量写在页签上，方便查看总数和已启用数。
- 设置页界面更统一，并跟随系统亮暗。
- 专家团卡片不再提供「复制提示词」。需要时在详情里查看主理人提示词。
