...
export function CustomExpertEditor(props: CustomEditorProps): React.ReactElement {
  ...
  h('footer', { className: 'aag-custom-footer', inert: discard ? '' : undefined },
    ...
    h('div', null, h('button', { type: 'button', className: 'aag-action', disabled: busy, onClick: close }, props.t('custom.cancel')),
      draft.slug ? null : h('button', { type: 'button', className: 'aag-action', disabled: busy || needsReview, onClick: () => submit(false) }, props.t('custom.save')),
      h('button', { type: 'button', className: 'aag-action aag-custom-primary', disabled: busy || needsReview, onClick: () => submit(draft.slug ? enabled : true) }, props.t(busy ? 'custom.saving' : draft.slug ? 'custom.saveChanges' : 'custom.saveEnable')))),
