...
describe('独立插件更新', () => {
  ...
  it('客户端与 Host 都只绑定专家库自身更新入口', async () => {
    ...
    expect(updateUi).toContain('box-shadow:var(--dsw-elevation-prominent)')
    expect(updateUi).toContain('border-radius:24px')
    expect(updateUi).toContain('background:var(--dsw-alias-bg-mask-1)')
