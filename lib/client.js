	...
	factory: (require) => {
		...
		*/
		...
		async function readEnabled(api) {
			...
			return {
				enabled: extractEnabled(view),
				revision: view.revision,
				writable: response.result.value.writable
			};
		}
		async function writeEnabled(api, enabled, revision) {
			...
			if (!response.result.ok) throw new Error(response.result.error.message);
			return {
				enabled: extractEnabled(response.result.value),
		...
		function AgentsButton(props) {
			...
			const onClick = () => {
				readEnabled(props.api).then((state) => {
					const current = state?.enabled ?? /* @__PURE__ */ new Set();
					setEnabled(current);
					if (current.size === 0) {
			...
			const pick = (slug) => {
				...
				const instruction = props.t(hasTask ? "summon.instruction.withTask" : "summon.instruction", {
					name,
					slug,
					task: draft
				});
		...
		function AgentsSettings(props) {
			...
			react.default.useEffect(() => {
				...
				readEnabled(props.api).then((state) => {
					if (!alive) return;
					setEnabled(state?.enabled ?? /* @__PURE__ */ new Set());
			...
			const toggle = (slug) => {
				...
				setEnabled(next);
				writeEnabled(props.api, next, revision).then((state) => {
			...
			if (error !== null) return react.default.createElement("div", { className: "aag-error" }, error);
			if (enabled === null) return react.default.createElement("div", { className: "aag-loading" }, props.t("settings.loading"));
			const groups = groupByDivision(EXPERTS);
			return react.default.createElement("div", { className: "aag-settings" }, groups.map((g) => react.default.createElement("div", {
				key: g.division,
				className: "aag-group"
			}, react.default.createElement("div", { className: "aag-group-title" }, props.t("group.count", {
				division: props.t(`division.${g.division}`),
				count: g.experts.length
			})), g.experts.map((e) => react.default.createElement("div", {
				key: e.slug,
				className: "aag-row"
			}, react.default.createElement("span", { className: "aag-emoji" }, e.emoji), react.default.createElement("div", { className: "aag-info" }, react.default.createElement("div", { className: "aag-name" }, displayName(e, props.getActive())), react.default.createElement("div", { className: "aag-desc" }, displayDescription(e, props.getActive()))), react.default.createElement("button", {
				type: "button",
				className: `aag-toggle ${enabled.has(e.slug) ? "aag-on" : "aag-off"}`,
				onClick: () => toggle(e.slug)
			}, props.t(enabled.has(e.slug) ? "settings.enabled" : "settings.disabled")))))));
		}
		const inject = [
			"slots",
			"inputTriggers",
			...
			for (const [i, div] of DIVISION_ORDER.entries()) {
				const source = {
					...
					candidates: async (_session, req) => {
						const enabled = (await readEnabled(api).catch(() => null))?.enabled ?? /* @__PURE__ */ new Set();
