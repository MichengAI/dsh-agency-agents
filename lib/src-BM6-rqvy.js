...
function isNewerVersion(currentValue, candidateValue) {
	...
	if (candidate.prerelease.length === 0 || current.prerelease.length === 0) return candidate.prerelease.length === 0 && current.prerelease.length > 0;
		...
		handler: async (request, response) => {
			try {
				...
				if (!isTrustedUpdateRequest(request)) {
					json(response, 403, { error: "已拒绝非同源更新请求。" });
					return;
				}
				if (installing) {
				...
				if (typeof process.send === "function") setTimeout(() => {
...
//#endregion
export { validateSummonSpecs as C, ZH_NAME as D, settingsNamespaceCompat as E, unquote as S, readHostLocale as T, resolveExpert as _, SUMMON_EXPERTS_MAX as a, toSummonItemResult as b, createAgencyPersonaSource as c, mapPool as d, name as f, resolveCatalogRoot as g, readLocalizedExpertPrompt as h, SUMMON_EXPERTS_CONCURRENCY as i, inject as l, readExpertPrompt as m, Config as n, SUMMON_TASK_MAX_CHARS as o, parseFrontmatter as p, DEFAULT_DIVISIONS as r, apply as s, AGENCY_PERSONA_SERVICE as t, loadCatalog as u, sanitize as v, formatHost as w, truncate as x, stripBom as y };
