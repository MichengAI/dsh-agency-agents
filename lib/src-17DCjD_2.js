...
function validateAgencySettings(value) {
	z.array(customExpertSchema).max(200).parse(value.customExperts ?? []);
...
function createExpertLibrary(base, store, divisions, locale) {
	...
	const assertUnique = (items) => {
		...
		for (const expert of items) {
			...
			for (const name of [expert.name, expert.nameEn]) {
				...
				if (owner !== void 0 && owner !== expert.slug) throw customError("duplicate", locale());
	...
	const library = {
		async catalog() {
			...
			return {
				experts,
				enabled: state.enabled.filter((slug) => available.has(slug)),
		...
		async saveCustom(input, enabled, expectedRevision) {
			...
			if (value.slug === void 0 && state.customExperts.length >= 200) throw customError("limit", locale());
			const expert = {
				...value,
		...
		async deleteCustom(slug, expectedRevision) {
			...
			return persist({
				customExperts: state.customExperts.map((item) => item.slug === slug ? {
					...item,
					deleted: true,
					wasEnabled: state.enabled.includes(slug)
				} : item),
		...
		async restoreCustom(slug, expectedRevision) {
			...
			if (expert === void 0) throw customError("missing", locale());
			const next = state.customExperts.map((item) => item.slug === slug ? {
				...item,
				deleted: false
			} : item);
			assertUnique([...builtins, ...next.filter((item) => !item.deleted).map(summary)]);
			return persist({
				customExperts: next,
				enabled: [...state.enabled, ...expert.wasEnabled ? [slug] : []]
			}, expectedRevision);
		}
	};
	return library;
...
function createAgencyPersonaSource(root, divisions) {
	const chineseRoot = resolve(root) === resolve(BUNDLED_ROOT) ? BUNDLED_CHINESE_ROOT : void 0;
	return { getPrompt: (slug, division, locale) => readLocalizedExpertPrompt(root, chineseRoot, slug, division, locale, divisions) };
}
...
//#endregion
export { settingsNamespaceCompat as A, validateSummonSpecs as C, expertEditSchema as D, customExpertInputSchema as E, formatHost as O, unquote as S, catalogSnapshotSchema as T, resolveExpert as _, SUMMON_EXPERTS_MAX as a, toSummonItemResult as b, createAgencyPersonaSource as c, mapPool as d, name as f, resolveCatalogRoot as g, readLocalizedExpertPrompt as h, SUMMON_EXPERTS_CONCURRENCY as i, ZH_NAME as j, readHostLocale as k, inject as l, readExpertPrompt as m, Config as n, SUMMON_TASK_MAX_CHARS as o, parseFrontmatter as p, DEFAULT_DIVISIONS as r, apply as s, AGENCY_PERSONA_SERVICE as t, loadCatalog as u, sanitize as v, AGENCY_LIBRARY_SERVICE as w, truncate as x, stripBom as y };
