...
declare function readExpertPrompt(root: string, slug: string, division: string, divisions?: readonly string[]): Promise<{
  prompt: string;
}>;
/** 按界面语言读取 persona；没有中文目录或中文译文时回退主目录正文。 */
declare function readLocalizedExpertPrompt(root: string, chineseRoot: string | undefined, slug: string, division: string, locale: LocaleId, divisions?: readonly string[]): Promise<{
  prompt: string;
}>;
interface AgencyPersonaSource {
  getPrompt(slug: string, division: string, locale: LocaleId): Promise<{
    prompt: string;
  }>;
}
...
//#endregion
export { toSummonItemResult as C, ZH_NAME as D, validateSummonSpecs as E, stripBom as S, unquote as T, readExpertPrompt as _, SUMMON_EXPERTS_CONCURRENCY as a, resolveExpert as b, SummonExpertItemResult as c, createAgencyPersonaSource as d, inject as f, parseFrontmatter as g, name as h, DEFAULT_DIVISIONS as i, SummonExpertSpec as l, mapPool as m, AgencyPersonaSource as n, SUMMON_EXPERTS_MAX as o, loadCatalog as p, Config as r, SUMMON_TASK_MAX_CHARS as s, AGENCY_PERSONA_SERVICE as t, apply as u, readLocalizedExpertPrompt as v, truncate as w, sanitize as x, resolveCatalogRoot as y };