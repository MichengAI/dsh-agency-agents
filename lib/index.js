...
*/
var AgencyAgentsRemote = class extends TypertRemoteService {
	static inject = ["settings"];
	constructor(ctx) {
		super(ctx, "agencyAgents");
	}
	/** 读取当前启用的专家 slug 列表。 */
	@Remote("getEnabled") getEnabled() {
		const enabled = this.ctx.settings.get(settingsNamespace("agency-agents"))?.enabled;
		return Array.isArray(enabled) ? enabled.filter((s) => typeof s === "string") : [];
	}
	/** 整体替换启用的专家 slug 列表。 */
	@Remote("setEnabled") async setEnabled(enabled) {
		await this.ctx.settings.mutate(settingsNamespace("agency-agents"), [{
			op: "set",
			path: ["enabled"],
			value: enabled
		}]);
