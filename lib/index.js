...
function resolveExpert(experts, query) {
	...
	const matches = exactNames.length > 1 ? exactNames : experts.filter((expert) => expert.slug.includes(q) || expert.name.toLowerCase().includes(q));
...
function apply(ctx, config) {
	...
	ctx.tools.register(defineTool({
		...
		async execute(args, exec) {
			...
			if (config.maxDepth !== void 0 && !provider.capabilities.depthLimit) throw new Error(`subagent provider "${config.provider}" does not support maxDepth`);
