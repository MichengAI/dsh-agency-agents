import { a as readHostLocale, c as installSettingsSectionCompat, d as ZH_NAME, i as matchDivision, l as settingsNamespaceCompat, n as localizedExpertDescription, o as renderExpertList, r as localizedExpertName, s as renderSummonResults, t as formatHost, u as ZH_DIVISION } from "./i18n-D6bVPfHY.js";
...
async function loadCatalog(root, divisions) {
	...
	for (const expert of map.values()) for (const name of [expert.name, expert.nameEn]) {
		...
		if (owner !== void 0 && owner.slug !== expert.slug) throw new Error(`花名册包含重复专家名称：${name}`);
...
function resolveExpert(experts, query, locale = "zh") {
	...
	const matches = exactNames.length > 1 ? exactNames : experts.filter((expert) => expert.name.toLowerCase().includes(q) || expert.nameEn?.toLowerCase().includes(q));
	if (matches.length === 1) return matches[0];
	if (matches.length > 1) {
		const preview = matches.slice(0, 12).map((expert) => locale === "en" ? expert.nameEn ?? expert.name : expert.name).join(", ");
