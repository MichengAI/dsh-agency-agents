...
在「设置 → 专家」点击「新建专家」，填写名称、简介、分类和专家提示词，可分别选择头像与召唤 Emoji（默认 🧩）。「保存并启用」后立即通过 @ 或专家按钮选择；标签与内置专家一致，显示统一专家图标和纯名称；Emoji 用于候选菜单。基础专家可复制为自定义，自定义专家可编辑、启停、删除，并在当前界面撤销最近一次删除。

数据保存在 DSH settings 的 agency-agents 命名空间，独立于插件目录，升级保留。外部 root 仍替换基础名册，自定义专家追加到该名册。最多 200 条自定义记录，包含软删除记录。多窗口冲突时保留输入并提示重新核对；停用或删除的草稿标签需移除或重新启用。写入携带修订号，协议以 [Remote 契约](src/remote-contract.ts) 为准。回退旧插件前应备份最新 settings，自定义数据的恢复与代码回退分开处理。
