...
数据保存在 DSH settings 的 agency-agents 命名空间，独立于插件目录，升级保留。外部 root 仍替换基础名册，自定义专家追加到该名册。最多保留 200 位自定义专家，删除立即释放名额；旧版已删除记录在启动及后续写入时清理。内置或外部目录更新导致的名称冲突会在设置页标记，允许改名或删除，冲突专家暂停召唤。多窗口冲突时保留输入并提示重新核对；停用或删除的草稿标签需移除或重新启用。写入携带修订号，协议以 [Remote 契约](src/remote-contract.ts) 为准。回退旧插件前应备份最新 settings，自定义数据的恢复与代码回退分开处理。
