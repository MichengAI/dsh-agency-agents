...
> 本 README 描述 1.0.1，正式发行以 npm 和 GitHub Releases 为准。完整改动见[更新日志](CHANGELOG.zh-CN.md)，双语版本说明见[发行说明](RELEASE_NOTES.md)。升级保留已有专家配置；回退旧版前先备份设置，旧版不具备团队功能。
## 界面预览
...
1. 点击对话输入框的「专家」，默认按分类显示已启用专家，保留 Emoji 和专家名称的紧凑列表；分区顺序沿用旧版，分区内按当前语言名称排序。
