...
try {
  ...
  check('npm 受信发布先执行独立门禁', workflow.includes('uses: ./.github/workflows/ci.yml') && workflow.includes('needs: check') && ci.includes('pnpm prepublishOnly'))
  check('npm 使用门禁构建且不重复运行生命周期', ci.includes('actions/upload-artifact@v4') && workflow.includes('actions/download-artifact@v4') && workflow.includes('npm publish --ignore-scripts'))
