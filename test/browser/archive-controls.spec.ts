...
for (const theme of ['dark', 'light']) test(`${theme}：沿用归档插件控件，启用状态保持绿色`, async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: '新建专家', exact: true })).toHaveCSS('border-radius', '14px')
  await expect(page.getByRole('switch', { checked: true }).first()).toHaveCSS('background-color', 'rgb(37, 207, 105)')
  await select.click()
  ...
  await expect(teamCard.getByRole('switch')).toHaveCSS('background-color', 'rgb(37, 207, 105)')
  ...
  await expect(firstSwitch).not.toHaveCSS('background-color', 'rgb(37, 207, 105)')
