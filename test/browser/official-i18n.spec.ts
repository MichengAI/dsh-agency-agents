...
for (const theme of ['dark', 'light']) test(`${theme}：控件使用官方主题变量及交互状态`, async ({ page }) => {
  ...
  await primary.hover()
  await expect(primary).toHaveCSS('background-color', 'rgb(96, 68, 187)')
  await page.locator('.aag-select-trigger').first().click()
  await expect(page.getByRole('option', { selected: true })).toHaveCSS('color', await page.locator('main').evaluate(el => getComputedStyle(el).getPropertyValue('--dsw-alias-label-primary').trim()).then(color => color === '#eee' ? 'rgb(238, 238, 238)' : 'rgb(32, 32, 32)'))
  await expect(page.locator('.aag-switch-track').first()).toHaveCSS('width', '36px')
...
test('聊天团队入口和异步确认使用英文词条与主题弹窗', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Expert teams', exact: true }).click()
