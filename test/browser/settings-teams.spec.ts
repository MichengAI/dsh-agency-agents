...
test('团队编辑保留停用状态，配置冲突先核对再保存', async ({ page }) => {
  ...
  await first.getByRole('menuitem', { name: '复制并自定义' }).click()
  ...
  await card.getByRole('menuitem', { name: '编辑专家团' }).click()
  ...
  await expect(card.getByRole('switch')).not.toBeChecked()
  await card.getByLabel(/更多/).click()
  await card.getByRole('menuitem', { name: '删除专家团' }).click()
...
test('专家团复用专家的筛选、卡片操作和编辑抽屉', async ({ page }) => {
  ...
  await card.getByLabel(/更多/).click()
  await card.getByRole('menuitem', { name: '复制并自定义' }).click()
...
test('切换专家团后保留专家搜索和来源筛选', async ({ page }) => {
  await page.goto('/?teams&settings')
  await page.getByRole('tab', { name: '内置', exact: true }).click()
  ...
  await expect(page.getByRole('tab', { name: '内置', exact: true })).toHaveAttribute('aria-selected', 'true')
...
test('切换团队保留同一个公共头部及宿主注入内容', async ({ page }) => {
  ...
  const tabsBefore = await page.getByRole('tablist', { name: '专家库类型' }).boundingBox()
  await page.getByRole('tab', { name: '专家团', exact: true }).click()
  await expect(header.locator('.mpi-version')).toHaveText('v1.0.0')
  await expect(header.locator('.mpi-check')).toBeVisible()
  await expect(page.locator('main .aag-title-row')).toHaveCount(1)
  expect(await header.boundingBox()).toEqual(before)
  expect(await page.getByRole('tablist', { name: '专家库类型' }).boundingBox()).toEqual(tabsBefore)
...
test('数量写在对应页签上，来源选项与新建同行', async ({ page }) => {
  ...
  for (const [tabName, createName, total] of [['专家', '新建专家', String(ROSTER.length)], ['专家团', '新建专家团', String(BUILTIN_TEAMS.length)]]) {
    ...
    const source = await page.getByRole('tab', { name: '内置', exact: true }).boundingBox()
    const create = await page.getByRole('button', { name: createName, exact: true }).boundingBox()
    expect(Math.abs(source!.y + source!.height / 2 - create!.y - create!.height / 2)).toBeLessThanOrEqual(8)
  }
